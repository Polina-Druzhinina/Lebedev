...
bool DataTime::correctData(int y, int m, int d){ //проверяет на корректность
    if (d < 1 || m < 1 || y < 1 || m > 12)
        return false;
...
string DataTime::doubleDigits(int value) {  //добавляет 0 в начало если это цифра 
...
ostream& operator<<(ostream& out, const DataTime& data){ //вывод в формате dd.mm.yyyy
    out<<data.toShort();
    return out;
}

string DataTime::formatTime(const string& hms){ 
    string res;
    int count = 0;
